...
function buildHistory(){
  ...
  document.getElementById('hist-filt').innerHTML =
    ['all','lesson','unit'].map(t=>`<button class="filt${histFilter===t?' on':''}" data-action="setFilt" data-arg="${t}">${{all:'All Quizzes',lesson:'Lesson Quizzes',unit:'Unit Quizzes'}[t]}</button>`).join('');
...
function renderScList(){
  ...
  list.innerHTML = filtered.map((s,i)=>{
    ...
    return `<div class="sc-card" style="border-left:4px solid ${cardAccent}" data-action="openScLightbox" data-arg="${i}"${_sr('role="button" aria-label="'+_escHtml(s.name)+', '+((s.abandoned||s.quit)?'did not finish':s.pct+'%, '+s.score+' of '+s.total)+', '+_escHtml(s.date)+'"')}>
...
function _buildScReviewHtml(s){
  ...
  const headHtml = `
    <button class="sc-lightbox-close" data-action="closeScLightbox">✕</button>
