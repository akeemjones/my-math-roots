...
// Node-only shim: the pure progress utilities this file uses (by bare name, both
// at top level and inside functions) now live in src/parent-progress.js. In the
// browser bundle and the test harness they are earlier-loaded globals; under a
// direct `require('../src/dashboard')` (some Jest suites) they are not, so pull
// them onto the global object here. No-op in the browser (require is undefined)
// and in the harness VM (require is not provided). Removed with this file.
if (typeof module !== 'undefined' && typeof require !== 'undefined' && typeof global !== 'undefined') {
  try {
    var _ppMod = require('./parent-progress');
    for (var _ppK in _ppMod) { if (global[_ppK] === undefined) global[_ppK] = _ppMod[_ppK]; }
  } catch (_ppErr) { /* bundle context — globals already present */ }
}

// ── Category mapping, grade-band helpers and settings parsers relocated to
//    src/parent-progress.js (used here by name from the bundle). ─────────────
...
// ── _draftSlot / _isUnitUnlockedInDraft / _isLessonUnlockedInDraft and the
//    _computeOverallStats/_computeSkillBreakdown/_computeWeakAreas pure
//    computations relocated to src/parent-progress.js. ───────────────────────
...
// (_last7DaysCutoffMs relocated to src/parent-progress.js)
...
// (_getLast7DaysLessonQuizScores relocated to src/parent-progress.js)
...
// (_computeReviewQueue relocated to src/parent-progress.js)
...
// ── Per-profile grade selection (_dbProfileGradeKey / _dbReadProfileGrade /
//    _dbWriteProfileGrade / _dbGradeBadge / _dbResolveProfileGrade) relocated
//    to src/parent-progress.js. ───────────────────────────────────────────────
...
// (_dbBuildStudentStreak / _dbBuildStudentActDates relocated to
//  src/parent-progress.js)
...
if (typeof module !== 'undefined') {
  // Pure data utilities relocated to src/parent-progress.js — require them from
  // there. What remains here is dashboard-specific (renderers + report/summary
  // helpers still tied to dashboard state), removed with the dashboard screen.
  module.exports = {
