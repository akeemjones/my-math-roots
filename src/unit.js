...
function _showLockedSheet(unitIdx, lockedIdx){
  ...
  ov.innerHTML = `
    <div class="locked-sheet" id="locked-sheet">
      ...
      <button class="locked-sheet-cta" style="background:${u.color}" data-action="_closeLockedSheetThenLesson" data-arg="${unitIdx}" data-arg2="${prevIdx}">
        ${prevPct > 0 ? '🔄 Try That Quiz Again' : '📖 Go to ' + prev.title}
      </button>
      <button class="locked-sheet-dismiss" data-action="_closeLockedSheet">Come Back Later</button>
    </div>`;
...
function openUnit(idx){
  ...
  u.lessons.forEach((l, i) => {
    ...
    if(unlocked){
      ...
      card.innerHTML = `
        ...
        <div class="lcard-badges">
          <button type="button" class="lcard-pin-btn" data-action="openLessonPinUnlock" data-arg="${idx}" data-arg2="${i}"
            title="Parent unlock"${_sr('aria-label="Parent unlock Lesson '+(i+1)+'"')}>${_ICO.lock}</button>
        </div>`;
      card.onclick = (e) => { if(!e.target.closest('.lcard-pin-btn')) _showLockedSheet(idx, i); };
    }
    lc.appendChild(card);
  });
  ...
  if(uqUnlocked && hasPausedUQ){
    ...
    uqResumeArea.innerHTML = `<div class="resume-banner" style="margin-bottom:12px;cursor:pointer" data-action="resumeQuiz" data-arg="${u.id}_uq">
      <div>
      ...
      <button type="button" class="resume-btn" data-action="resumeQuiz" data-arg="${u.id}_uq">▶ Resume</button>
    </div>`;
    ...
    if(uqPct >= 80 && hasNextUnit){
      ...
      uqResumeArea.innerHTML = `<div style="text-align:center;margin-bottom:6px">
        <button type="button" style="background:none;border:none;color:#999;font-size:.82rem;cursor:pointer;text-decoration:underline;font-family:inherit" data-action="startUnitQuiz" data-arg="${idx}">↩ Retake Unit Quiz</button>
      </div>`;
    } else {
...
function playCarryAnim(btn){
  ...
  stepsDiv.innerHTML = `
    <div class="cca-wrap" id="cca-root">
      ...
      <button class="cca-btn" id="cca-btn" style="background:#e67e22" data-action="advanceCarryAnim">▶ Start</button>
    </div>`;
...
function play3dCarry(btn, top, bot){
  ...
  stepsDiv.innerHTML=`
    <div class="cca-wrap">
      ...
      <button class="cca-btn" id="${p}-btn" style="background:#d35400" data-action="callBlockAdv" data-arg="${p}">▶ Start</button>
    </div>`;
...
function playBorrowAnim(btn){
  ...
  stepsDiv.innerHTML = `
    <div class="cca-wrap" id="ccb-root">
      ...
      <button class="cca-btn" id="ccb-btn" style="background:#e67e22" data-action="advanceBorrowAnim">▶ Start</button>
    </div>`;
...
function buildPracticeQ(pid, q){
  ...
  return `<div class="pq-drill" id="${pid}" data-correct="${_escHtml(correctText)}" data-exp="${_escHtml(q.e)}">
    ...
    <div class="pq-choices">
      ${opts.map((text, i) =>
        `<button class="pq-choice" type="button" id="${pid}-c${i}" data-action="_pickPracticeAns" data-arg="${pid}" data-arg2="${i}">${_escHtml(text)}</button>`
      ).join('')}
    </div>
...
function _renderLesson(unitIdx, lessonIdx){
  ...
  html += `<div class="learn-card" id="ex-card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      ...
      <button class="new-ex-btn" style="background:${u.color}" type="button"
        data-action="refreshExamples" data-arg="${unitIdx}" data-arg2="${lessonIdx}">✨ New Examples</button>
    </div>
    ...
    html += `<div class="practice-card" id="practice-card-${l.id}">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
        ...
        <button class="more-practice-btn" style="background:${u.color}" type="button"
          data-action="morePractice" data-arg="${unitIdx}" data-arg2="${lessonIdx}">➕ More</button>
      </div>
      <div id="pq-list-${l.id}">`;
  ...
  if(hasPausedQuiz){
    ...
    html += `<div class="resume-banner" style="cursor:pointer" data-action="resumeQuiz" data-arg="lq_${l.id}">
      <div>
      ...
      <button type="button" class="resume-btn" data-action="resumeQuiz" data-arg="lq_${l.id}">▶ Resume</button>
    </div>`;
  } else {
    html += `<button type="button" class="lq-banner lq-start-btn" data-action="startLessonQuiz" data-arg="${unitIdx}" data-arg2="${lessonIdx}" style="background:linear-gradient(135deg,${u.color},${u.color}cc);width:100%;border:none;cursor:pointer;font-family:inherit;text-align:center;">
  ...
  if(lessonPassed){
    ...
    if(!isLastLesson && nextUnlocked){
      html += `<div class="done-card" style="text-align:center">
        ...
        <button class="next-lesson-btn" style="border-color:${u.color};color:${u.color};margin-top:14px;width:100%"
          data-action="openLesson" data-arg="${unitIdx}" data-arg2="${nextLessonIdx}">
    ...
    html += `<button class="next-lesson-btn next-lesson-locked"
      data-action="showLockToast" data-arg="Score 80% or higher on this lesson quiz to unlock the next lesson!" data-arg2="true">
    ...
    if(uqUnlocked){
      html += `<button class="next-lesson-btn" style="border-color:${u.color};color:${u.color}"
        data-action="goUnit">📝 Go to Unit Quiz →</button>`;
    } else {
      const subMsg2 = lqBestPct > 0 ? `Best score: ${lqBestPct}% — need 80% to unlock` : 'Score 80% on the quiz to unlock';
      html += `<button class="next-lesson-btn next-lesson-locked"
        data-action="showLockToast" data-arg="Get 80% or higher on all lessons to unlock the unit quiz!">
          ${_ICO.lock} Unit Quiz
