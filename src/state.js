...
function _updateMastery(answers){
  ...
  answers.forEach(function(a){
    if(!a || !a.t) return;
    const k = _qId(a);
    // Lazy migration: move old text-hash entry to stable id key on first touch
    if(a.id && !MASTERY[k]){
      const oldKey = _qKey(a.t);
      if(oldKey !== k && MASTERY[oldKey]){ MASTERY[k] = MASTERY[oldKey]; delete MASTERY[oldKey]; }
    }
    const m = MASTERY[k] || { attempts:0, correct:0, lastSeen:0, nextReview:null };
    if(!('nextReview' in m)) m.nextReview = null;
    m.attempts++;
    if(a.ok) m.correct++;
    m.lastSeen = now;
    MASTERY[k] = m;
